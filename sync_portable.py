"""sync_portable.py - Sincroniza el proyecto con la carpeta portable."""
import os
import shutil
    ...
    for item in os.listdir(dst):
        ...
        if not os.path.exists(src_item):
            if os.path.isdir(dst_item):
                ...
                print(f"Eliminado obsoleto: {dst_item}")
            else:
                os.remove(dst_item)
                print(f"Eliminado obsoleto: {dst_item}")
...
def main():
    ...
    portable_dir = os.path.join(base_dir, "g360-erp-stock-consolidator-portable")
    ...
    files_to_sync = [
        "run.py",
        "run.bat", "launch.vbs", "launch_minimized.bat",
        "create_shortcut.vbs", "build-portable.bat",
        "README.md", "pyproject.toml", "requirements.txt",
        ".env.example", "skill.json",
    ]
    for filename in files_to_sync:
        ...
        if os.path.exists(src_file):
            ...
            print(f"Copiado: {filename} -> portable/")
        else:
